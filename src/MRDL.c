...
static int pre_work(struct worker *worker)
{
	...
	/* Completing one worker stops the whole benchmark. */
...
out:
	...
	fprintf(stderr, "# precreate MRDL worker=%d files=%" PRIu64 "\n",
...
out:
#if CXLFS_ENABLE_FXMARK_PRECREATE_REPORT
	fprintf(stderr, "# scans MRDL worker=%d opens=%" PRIu64 " complete=%" PRIu64 " entries=%" PRIu64 "\n",
