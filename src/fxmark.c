...
static struct bench_desc bench_table[] = {
	...
	{"MWCM",
	 ...
	 &u_file_cr_ops},
	{"MWUL",
	 "each process deletes a fixed number of files in their private directory",
	 &u_file_rm_ops},
	{"MWUL_ENOSPC",
	 "each process deletes files created until ENOSPC in their private directory",
	 &u_file_rm_enospc_ops},
	{"MWUM",
	 "each process deletes a fixed number of files at the test root directory",
	 &u_sh_file_rm_ops},
	{"MWUM_ENOSPC",
	 "each process deletes files created until ENOSPC at the test root directory",
	 &u_sh_file_rm_enospc_ops},
	{"DWTL",
	 "each process truncates a fixed-size private file at the test root directory",
	 &u_file_tr_ops},
	{"DWTL_ENOSPC",
	 "each process truncates its private file created until ENOSPC at the test root directory",
	 &u_file_tr_enospc_ops},
	{NULL, NULL, NULL},
};
